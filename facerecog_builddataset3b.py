...
recognizer.read('trainer\\trainer.yml')  #load training model
cascadePath = "haarcascade_frontalface_alt2.xml"  #cascade path
...
while True:
    ...
    for(x,y,w,h) in faces:
        ...
        if(Id == 2):    # Check the ID if exist 
            Id = "{0:.2f}%".format(round(100 - confidence, 2)) 
            Id = "Furqon + {0:.2f}%".format(round(100 - confidence, 2)) 
            print("Verified")
            print("Furqon")
            counter_correct += 1
            print("counter_correct: " + str(counter_correct))
            cv2.rectangle(im, (x-22,y-90), (x+w+22, y-22), (255,255,255), -1)
            cv2.putText(im, str(Id), (x,y-40), font, 1, (0,0,0), 2)
        #if(counter_wrong == 3000):
        #    pyautogui.moveTo(48,748)
        #    pyautogui.click(48,748)
        #    #pyautogui.typewrite("Hello Stranger!!! Whats Up.")
        #    print("Hello Stranger!!! Whats Up.")
        #    cam.release()
        #    cv2.destroyAllWindows()
        #    #ctypes.windll.user32.LockWorkStation()
        #    #sys.exit()
        #if(counter_correct == 600):    #if counter = 6 then program will terminate as it has recognized correct user for 6 times. 
        #    cam.release()
        #    cv2.destroyAllWindows()
        #    sys.exit()
